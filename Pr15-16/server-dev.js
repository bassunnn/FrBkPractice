...
// Используем обычный HTTP сервер вместо HTTPS
...
server.listen(PORT, () => {
  console.log(`HTTP сервер запущен (DEV режим): http://localhost:${PORT}`);
  console.log('Примечание: используется HTTP вместо HTTPS для локальной разработки');
});
